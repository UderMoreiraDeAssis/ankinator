...
**Status (2026-06-06):** ✅ **P2** (reconciliações de consistência) aplicadas e commitadas (`4338198`). ✅ **P0/P1** (recomendações de framing) aplicadas aos docs via **DEC-q** — Core Value reenquadrado (dono explícito), **Definição de Pronto** (gate ao vivo + eixos render×valor), barras de progresso render×valor, constraint de ops destrutivas. ⏳ Pendente de **execução** (não-doc): medir o sinal de retenção AO VIVO. (`live-validate.ts` + lote do orquestrador commitados em `664baa8`; UI em `6559e25`.)
